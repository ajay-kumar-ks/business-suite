...
def create_bill_journal(db: Session, tenant_id: int, bill) -> JournalEntry:
    """
    ...
    credit_line = JournalLine(
        ...
        memo=f"Payable to {bill.vendor.name if hasattr(bill, 'vendor') else 'Vendor'}",
        debit=Decimal("0"),
...
def create_vendor_payment_journal(db: Session, tenant_id: int, payment, bill) -> JournalEntry:
    """
    ...
    credit_line = JournalLine(
        ...
        memo=f"Payment to {bill.vendor.name if hasattr(bill, 'vendor') else 'Vendor'}",
        debit=Decimal("0"),
